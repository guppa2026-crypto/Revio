...
export default function DashboardPage() {
  ...
  const filtered = filter === 'all'
    ? reviews
    : reviews.filter(r => r.reply_status === filter)
  ...
  const stats: Stats = {
    ...
    avgRating: reviews.length
      ...
      : 0,
    pendingCount: reviews.filter(r => r.reply_status === 'pending').length,
    autoReplied: reviews.filter(r => r.reply_status === 'posted').length,
  }
    ...
    <>
      <style>{`
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        body { background: #F7F6F3; font-family: 'DM Sans', system-ui, sans-serif; }

        @import url('https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;600&family=DM+Mono:wght@400;500&display=swap');

        .shell { min-height: 100vh; background: #F7F6F3; }
        ...
        .nav-signout { font-size: 13px; color: #888; border: none; background: none; cursor: pointer; padding: 6px 10px; border-radius: 6px; transition: background 0.15s; }
        ...
        .filter-btn { font-size: 12px; font-weight: 500; padding: 5px 12px; border-radius: 99px; border: 1px solid #ECEAE4; background: #fff; color: #888; cursor: pointer; text-transform: capitalize; transition: all 0.15s; }
        .filter-btn:hover { border-color: #C8C4BC; color: #444; }
        .filter-btn.active { background: #1A1916; color: #fff; border-color: #1A1916; }

        /* Cards */
        .reviews { display: flex; flex-direction: column; gap: 12px; }
        .card { background: #fff; border: 1px solid #ECEAE4; border-radius: 14px; padding: 18px 20px; transition: border-color 0.15s; }
        .card:hover { border-color: #C8C4BC; }
        ...
        .reply-label span { color: #7F77DD; }
        .reply-text { font-size: 13px; color: #2C2C2A; line-height: 1.65; }
        textarea.reply-edit { width: 100%; font-size: 13px; color: #2C2C2A; line-height: 1.65; background: #fff; border: 1px solid #C8C4BC; border-radius: 8px; padding: 10px 12px; resize: vertical; min-height: 80px; font-family: inherit; outline: none; }
        textarea.reply-edit:focus { border-color: #7F77DD; }
        ...
        .btn { font-size: 13px; font-weight: 500; padding: 7px 16px; border-radius: 8px; cursor: pointer; border: 1px solid transparent; transition: all 0.15s; font-family: inherit; }
        ...
        .btn-edit:hover { border-color: #C8C4BC; }
        ...
        <div className="page">
          ...
          <div className="stats">
            ...
            <div className="stat">
              ...
              {stats.avgRating > 0 && (
                <div className="stat-sub">{'★'.repeat(Math.round(stats.avgRating))}{'☆'.repeat(5 - Math.round(stats.avgRating))}</div>
              )}
            </div>
            ...
            <div className="reviews">
              {filtered.map(review => {
                ...
                return (
                  <div
                    key={review.id}
                    className={`card ${review.reply_status === 'flagged' ? 'card-flagged' : ''}`}
                  >
                      ...
                      <div className="reviewer">
                        ...
                        <div>
                          ...
                          <div className="reviewer-meta">
                            {review.created_at ? timeAgo(review.created_at) : ''} · Google
                          </div>
                        </div>
                      </div>
                    ...
                    {review.reply_status === 'flagged' ? (
                      <div className="flagged-banner">
                        ⚠ Flagged for manual review — respond personally, do not auto-reply.
                      </div>
                    ) : replyText ? (
                      <div className="reply-box">
                        ...
                        {isEditing ? (
                          <textarea
                            ...
                            value={editing[review.id]}
                            onChange={e => setEditing(ed => ({ ...ed, [review.id]: e.target.value }))}
                          />
                        ) : (
                      ...
                      <div className="actions">
                        ...
                        {!isEditing && (
                          <button
                            ...
                            onClick={() => setEditing(e => ({ ...e, [review.id]: review.ai_reply }))}
                          >
                            Edit reply
                          </button>
                        )}
