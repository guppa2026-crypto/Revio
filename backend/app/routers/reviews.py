...
def regenerate_reply(
    ...
    new_reply = generate_reply(review.review_text or "", review.rating, tenant.name, risk_level=risk_level, tone_guidance=tenant.tone_guidance or "", sample_replies=tenant.sample_replies or "")
...
def import_review(
    ...
    db.add(review)
    db.flush()
    processed = process_review(review, db, tenant.name, tone_guidance=tenant.tone_guidance or "", sample_replies=tenant.sample_replies or "")
    return processed
...
def test_process_review(
    ...
    db.add(fake_review)
    db.flush()
    processed = process_review(fake_review, db, tenant.name, tone_guidance=tenant.tone_guidance or "", sample_replies=tenant.sample_replies or "")
    return {
